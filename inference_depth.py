...
def sample_illusion_image(output_dir, latent, text_prompt, decode_steps=100, direct_transfer_steps=60, decayed_transfer_steps=0,
    ...
    x_rec = sampler.decode_with_phase_substitution(output_dir=output_dir, ref_latent=latent, cond=cond, t_dec=decode_steps,
...
latents_ref_inversion = inversion(load_ref_img(image_path, contrast=contrast, add_noise=add_noise), uncond, diffusion_model)
...
set_random_seed(99)
sample = sample_illusion_image(output_dir=output_dir, latent=load_inverted_noise(), direct_transfer_steps=direct_transfer_steps, decayed_transfer_steps=decayed_transfer_steps, text_prompt=prompt)
